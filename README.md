...
1. 한국 교통 표지판의 데이터 셋은 생각보다 많이 구하기 어렵고, 데이터 분할이 제대로 진행된 데이터 셋이 없었음.
2. 데이터 셋이 그나마 분할이 잘 되어 있다고 판단된 데이터 셋을 구하게 됨.
3. 하지만 어렵게 구한 데이터 셋마저 Yolo에서 실행 파일로 사용되는 data.yaml 파일에 클래스 오류가 많았음
